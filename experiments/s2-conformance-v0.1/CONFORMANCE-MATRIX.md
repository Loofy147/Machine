...
|---|---|---|---|
| S2-C01 | Present key produces \`Hit(v)\` with the abstract stored value | §4, §15 | hit/oracle |
| S2-C02 | Absent key produces \`Miss\` | §4, §11, §15 | miss/oracle |
...
There are currently 13 executable semantic test cases for these 15 requirement IDs; a test case may discharge multiple requirements.

All 15 IDs have at least one executable assertion. C13 is explicitly bounded to two concrete Mapping representations, and C14/C15 are implementation-adapter tests for the declared experimental target boundary.
