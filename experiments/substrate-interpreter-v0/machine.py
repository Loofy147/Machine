...
class Interpreter:
    ...
    def step(self, st):
        ...
        if isinstance(c,(Closure,Primitive,Pair,IndexedRelation,Unit,bool,int,float,str,tuple)):
            self.cont(st); return
    ...
    def prim(self,name,args,st):
        ...
        if name=="iter":
            if len(args)!=1 or not isinstance(args[0],tuple): raise LanguageError("iter")
            if st: st.access_ticks += len(args[0])
            return args[0]
        if name=="relation_lookup_direct":
            if not self.contract.direct_relation_lookup: raise LanguageError("direct lookup unavailable")
            if len(args)!=2 or not isinstance(args[0],IndexedRelation): raise LanguageError("direct lookup")
            if st: st.access_ticks += 1
            return args[0].index.get(args[1],NIL)
        raise LanguageError(f"unknown primitive: {name}")

def build_indexed_relation(entries: Sequence[Pair]):
    e=tuple(entries); idx={x.head:x.tail for x in e}
    return IndexedRelation(e,idx),len(e),len(idx)
...
def direct_lookup(indexed: IndexedRelation,key):
    return indexed.index.get(key,NIL),1
