...
def verify_error_source(entry: dict) -> None:
    ...
    # The harness emits by-mode aggregates only to stdout, so rerun the
    # deterministic program and derive the same aggregate shape from JSON
    # output generated by the checked-in version if it is changed later.
    # For v0, the executable's documented aggregate values are asserted from
    # the protocol record via an independent in-process reproduction below.
    expected = entry["expected"]

    # Re-run through the script's deterministic behavior by importing the
    # checked-in module. This deliberately uses the same source file, while
    # keeping the assertion logic outside the harness.
    ...
    policies = {
        m: module.Policy(m, 20260916 + offset, shared if m == "adaptive" else None)
        for m, offset in zip(("local", "random", "probe", "adaptive"), (23, 37, 53, 71))
    }
    rows = []
    for ep in range(100):
        fault = module.FAULTS[ep % len(module.FAULTS)]
        for mode in ("local", "random", "probe", "adaptive"):
            rows.append(module.run_episode(fault, policies[mode], 4))
    for mode, exp in expected.items():
        actual_rows = [r for r in rows if r["mode"] == mode]
        if len(actual_rows) != 100:
            fail(f"error-source {mode}: expected 100 rows")
        actual = {
            "recovery_rate": sum(r["recovered"] for r in actual_rows) / 100,
            "localization_rate": sum(r["localized"] for r in actual_rows) / 100,
            "mean_recovery_cost": sum(r["cost"] for r in actual_rows) / 100,
            "mean_final_test": sum(r["final_test"] for r in actual_rows) / 100,
        }
        for key, expected_value in exp.items():
            if actual[key] != expected_value:
                fail(f"error-source {mode}.{key}: expected {expected_value}, got {actual[key]}")
